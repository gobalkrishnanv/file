...
import com.niit.microcontroller.Service.SupplierService;
...
import com.niit.microcontroller.model.Supplier;
...
public String loiga(@ModelAttribute("user") User u,Model m) {
	...
	{
		return "productfrom";
	}else if (s.getName()==name && s.getPassword()==pass){
		return "register";
	}else {
		return "admin";
...
public String prod(Model m) {
	  m.addAttribute("s_product", "Product Form:");

return "productfrom";
}

@RequestMapping(value="/psave",method=RequestMethod.POST)
public String prodreg(@ModelAttribute("prod") Product u,Model m) {
	  m.addAttribute("s_product", "Product Form:");

	if(u!=null) {
		
		String fa=u.getName()+".jpg";
		
		File filepath=new File("C:\\Users\\Gobal Krishnan\\Desktop\\A\\Arduino\\src\\main\\webapp\\resources\\images\\" +fa);
        
		try {
			FileCopyUtils.copy(u.getMultipart().getBytes(),filepath);
			byte[] buffer=u.getMultipart().getBytes();
